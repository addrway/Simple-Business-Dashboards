...
function ProtectedApp({ page, setPage }) { const { session, trialActive } = useAuth(); if (!session) return <AuthPage mode="login" setPage={setPage} />; if (!trialActive()) return <Expired setPage={setPage} />; return <DashboardShell active={page} setPage={setPage} />; }

function DashboardShell({ active, setPage }) {
  const { signOut, hoursLeft, planLabel } = useAuth();
